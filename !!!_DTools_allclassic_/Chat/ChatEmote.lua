...
emotes = {
	...
	{ key = "{rt1}", zhTW="rt1", zhCN="星形",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_1" },
	{ key = "{rt2}", zhTW="rt2", zhCN="大饼",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_2" },
	{ key = "{rt3}", zhTW="rt3", zhCN="菱形",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_3" },
	{ key = "{rt4}", zhTW="rt4", zhCN="三角",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_4" },
	{ key = "{rt5}", zhTW="rt5", zhCN="月亮",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_5" },
	{ key = "{rt6}", zhTW="rt6", zhCN="方块",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_6" },
	{ key = "{rt7}", zhTW="rt7", zhCN="红叉",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_7" },
	{ key = "{rt8}", zhTW="rt8", zhCN="骷髅",	texture = "Interface\\TargetingFrame\\UI-RaidTargetingIcon_8" },
...
local function ReplaceEmote(value)
	...
	for _, v in ipairs(emotes) do
		if (emote == v.key or emote == v.zhCN or emote == v.zhTW) then
			return "|T"..(v.texture or "Interface\\AddOns\\"..AddonName.."\\media\\emotes\\"..v.key)..":16|t"
		end
	end
	return value
end
...
do
	local frame, button
	local width, height, column, space = 26, 26, 12, 5
	local index = 0
	frame = CreateFrame("Frame", "CustomEmoteFrame", UIParent, "BackdropTemplate")

	frame.Title = frame:CreateFontString(nil, "ARTWORK")
	frame.Title:SetFont(STANDARD_TEXT_FONT, 20, "OUTLINE")
	frame.Title:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -11, 13)
	...
	frame:SetPoint("BOTTOMRIGHT", ChatEmote, "TOPRIGHT", -4, -7)--這裡調整位置
	for _, v in ipairs(emotes) do
		...
		if (v.texture) then
			...
			button:SetNormalTexture("Interface\\AddOns\\"..AddonName.."\\media\\emotes\\"..v.key)
		end
		button:SetHighlightTexture("Interface\\Buttons\\UI-Common-MouseHilight", "ADD")
		button:SetPoint("TOPLEFT", 10+(index%column)*(width+space), -10-floor(index/column)*(height+space))
	...
	frame:SetWidth(column*(width+space) + 16)
	frame:SetHeight(ceil(index/column)*(height+space) + 16)
	-- frame:SetAlpha(.8)
	frame:Hide()
...
if (GetCVarBool("chatBubbles")) then
	...
	frame:SetScript("OnUpdate", function(self, elapsed)
		...
		if (not self.paused and self.timer > 0.2) then
			self.timer = 0
