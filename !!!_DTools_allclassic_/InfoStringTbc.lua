	...
	f:SetSize(100,100)
	f:SetPoint("BOTTOM", UIParent, "BOTTOMRIGHT", -240, 1)
	...
	infocfg.ttpos = { a1 = "BOTTOMLEFT", af = f, a2 = "BOTTOM", x = -260, y = 0 }
...
ns.event("PLAYER_LOGIN", function()
	...
	if DToolsDB.ifs == true then
		...
		local function LootSpec()
			...
			local _, _, _, _, num1 = GetTalentTabInfo(1)
			local _, _, _, _, num2 = GetTalentTabInfo(2)
			local _, _, _, _, num3 = GetTalentTabInfo(3)
			local usetalent
			...
			local cur_specID, cur_specName, _, cur_Icon = GetTalentTabInfo(usetalent)
			return "|c"..classcs..(L.spec or "天赋")..":|r".."|cff00ff00"..cur_specName.."|r"
		...
		local function Attributetype()
			...
			if stat.str > stat.int or stat.agi > stat.int then
				fa1abtt = L.attp or "攻强"
				fa4abtt = L.atts or "攻速"
			else
				fa1abtt = spellSuffix
				fa4abtt = L.manar or "法回"
			end
		...
		local function Attribute()
			...
			if stat.str > stat.int or stat.agi > stat.int then
				...
				fa1abt = stat.SpellDamage
				fa4abt = stat.ManaRegen
			end
		...
		if infocfg.showSpec then
			...
			----天赋切换
			f3:SetScript("OnMouseUp", function(self, button)
				if InCombatLockdown() then return end
			...
			-- 			for i = 1, numspec do
			-- 				local id, name, _, icon = GetTalentTabInfo(i)
			-- 				info.text = name
